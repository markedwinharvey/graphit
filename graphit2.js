$(document).on('click','#movedots',function(){
	...
	for(i=0;i<200;i+=20){
		...
		$('.points').css({'background-color':'rgb('+i+','+i+','+i+')'});
		console.log('here');
	}
});
...
function addPoint(x,y,pSize){
	...
	if (pLeft > 0 && pLeft < 800 && pTop > 0 && pTop < 800){
		...
		point.style.zIndex = zIndex;
		//zIndex++;
		point.className = 'points';		
		point.id = 'id'+point_id_num;
		point_id_num++;
		$('#graph').append(point);
		if (point_id_num > $('#maxDots').val()){
			$('#id'+disappearing_id).remove();
			disappearing_id++;
		}
	}
